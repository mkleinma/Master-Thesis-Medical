...
def save_confusion_matrix(y_true, y_pred, classes):
    ...
    cm_dict = {classes[i]: {classes[j]: int(cm[i][j]) for j in range(len(classes))} for i in range(len(classes))}

    with open(os.path.join(data_dir, "confusion_matrix.json"), "w") as f:
        json.dump(cm_dict, f, indent=4)

data_dir = "D:/data"

#data_dir = "/pfs/work7/workspace/scratch/ma_mkleinma-thesis/data"
