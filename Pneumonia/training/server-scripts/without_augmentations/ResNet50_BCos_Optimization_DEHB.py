...
logging.basicConfig(filename="train_debug_dehb.log", level=logging.DEBUG, format="%(asctime)s - %(levelname)s - %(message)s")
...
'''def train_model(config: Union[ConfigSpace.Configuration, List, np.array], fidelity: Union[int, float] = None, **kwargs) -> Dict:
    ...
    for fold, (train_idx, val_idx) in enumerate(splits):
        ...
        for epoch in range(int(fidelity)):
            print(f"Starting epoch {epoch+1}/{int(fidelity)}", flush=True)
            model.train()
            for images, labels in train_loader:
                labels = labels.to(device)
    ...
    print(f"Final avg_f1: {avg_f1}, avg_val_accuracy: {avg_val_accuracy} before returning default values", flush=True)
...
de = DEHB(f=train_model,
    ...
    n_workers=1)
incumbent = de.run(fevals=10, runtime=86400)
